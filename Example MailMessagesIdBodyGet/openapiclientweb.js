			...
			success: function(response) {
				...
				if (response) {
					...
					while (iter < response.messages.length){
						...
						row.cells[0].innerHTML = '<button onclick="getMailMessageBody('+(iter+1)+')"> Body '+iter+'</button>';
						iter = iter + 1;
					}
				}
			}
...
function getMailMessageBody(cid){
	...
	$.ajax({
			...
			url: apiHost + '/mail/messages/'+id+'/body?access_token='+$('span.token').text(),
			type: "GET",
			dataType: "json",
			success: function(response) {
				var table = document.getElementById("taulaBody");
				...
				if (response) {
					...
					firstrow.cells[1].innerHTML = "<b>body</b>";
					iter = 0;
					var i = 0;
					var row = table.insertRow(-1);
					for(i; i < 2; i++) {
					...
					row.cells[1].innerHTML = response.body;
				}
