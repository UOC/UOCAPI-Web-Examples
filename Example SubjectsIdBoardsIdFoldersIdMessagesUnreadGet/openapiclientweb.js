			...
			success: function(response) {
				...
				if (response) {
					...
					while (iter < response.folders.length){
						...
						row.cells[4].innerHTML = '<button onclick="getMessageL('+(iter+1)+')">getUnreadfromFolder'+iter+'</button>';
						iter = iter + 1;
					}
				}
			}
...
function getMessageL(index){
	...
	$.ajax({
			...
			url: apiHost + '/subjects/'+$('span.cid').text()+'/boards/'+$('span.bid').text()+'/folders/'+$('span.fid').text()+'/messages/unread?access_token='+$('span.token').text(),
			type: "GET",
			dataType: "json",
			success: function(response) {
