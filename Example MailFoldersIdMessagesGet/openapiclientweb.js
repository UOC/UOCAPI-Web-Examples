			...
			success: function(response) {
				...
				if (response) {
					...
					while (iter < response.folders.length){
						...
						row.cells[4].innerHTML = '<button onclick="getMailFoldersMessages('+(iter+1)+')">Messages '+iter+'</button>';						
						iter = iter + 1;
					}
				}
			}
...
function getMailFoldersMessages(cid){
	...
	$.ajax({
			...
			dataType: "json",
			success: function(response) {
				var table = document.getElementById("taulaFoldersMessages");
