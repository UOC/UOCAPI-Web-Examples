			...
			success: function(response) {
				...
				if (response) {
					...
					while (iter < response.messages.length){
						...
						row.cells[10].innerHTML = '<button onclick="getMessage('+(iter+1)+')">getMessagesHistory'+iter+'</button>';
						iter = iter + 1;
					}
				}
				$('div.getFolderL').hide();
...
function getMessage(index){
	...
	$.ajax({
			...
			url: apiHost + '/subjects/'+$('span.cid').text()+'/boards/'+$('span.bid').text()+'/folders/'+$('span.fid').text()+'/messages/'+$('span.mid').text()+'/history?access_token='+$('span.token').text(),
			type: "GET",
			dataType: "json",
			success: function(response) {
