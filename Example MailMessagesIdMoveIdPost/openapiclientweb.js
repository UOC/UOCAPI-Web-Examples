			...
			success: function(response) {
				...
				if (response) {
					...
					while (iter < response.messages.length){
						...
						row.cells[0].innerHTML = '<button onclick="getFolders('+(iter+1)+')">Move '+iter+'</button>';
						iter = iter + 1;
					}
				}
			}
...
function getFolders(mid){
	...
	//var id = tableElement.rows[mid].cells[1].innerHTML;
   	$.ajax({
			...
			success: function(response) {
				...
				if (response) {
					...
					while (iter < response.folders.length){
						...
						row.cells[0].innerHTML = '<button onclick="moveMail('+mid+','+(iter+1)+')">Move '+iter+'</button>';
						iter = iter + 1;
					}
				}
...
function moveMail(mid, fid){
	var tableElement = document.getElementById("taulaMessages");
	var id = tableElement.rows[mid].cells[1].innerHTML;
	var tableElement2 = document.getElementById("taulaFolders");
	var id2 = tableElement2.rows[fid].cells[1].innerHTML;
    /*STARTUOCAPIEXAMPLE*/
    var missatge = '{}'
	$.ajax({
			/* UOCAPICALL /api/v1/mail/messages/{id}/move/{folder_id} POST*/
			url: apiHost + '/mail/messages/'+id+'/move/'+id2+'?access_token='+$('span.token').text(),
			type: "POST",
			dataType: "json",
			data:"",
