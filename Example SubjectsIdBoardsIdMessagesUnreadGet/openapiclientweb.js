			...
			success: function(response) {
				...
				if (response) {
					...
					while (iter < response.boards.length){
						...
						row.cells[7].innerHTML = '<button onclick="getMessageL('+(iter+1)+')">getUnreadfromBoard'+iter+'</button>';
						iter++;
					}
				}
...
function getMessageL(index){
	...
	$.ajax({
			...
			url: apiHost + '/subjects/'+$('span.cid').text()+'/boards/'+$('span.bid').text()+'/messages/unread?access_token='+$('span.token').text(),
			type: "GET",
			dataType: "json",
			success: function(response) {
