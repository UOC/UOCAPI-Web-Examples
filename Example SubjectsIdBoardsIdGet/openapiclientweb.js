			...
			success: function(response) {
				...
				if (response) {
					...
					while (iter < response.boards.length){
						...
						row.cells[7].innerHTML = '<button onclick="getBoard('+(iter+1)+')">getBoardsfromSubj'+iter+'</button>';
						iter++;
					}
				}
			}
...
function getBoard(index){
	...
	$.ajax({
			...
			url: apiHost + '/subjects/'+$('span.cid').text()+'/boards/'+$('span.bid').text()+'?access_token='+$('span.token').text(),
			type: "GET",
			dataType: "json",
			success: function(response) {
