			...
			success: function(response) {
				...
				if (response) {
					...
					while (iter < response.folders.length){
						...
						row.cells[0].innerHTML = '<button onclick="moveMessage('+mid+','+(iter+1)+')">MoveMoessage '+iter+'</button>';
						iter = iter + 1;
					}
				}
...
function moveMessage(indexm, indexd){
	...
	$.ajax({
			...
			url: apiHost + '/classrooms/'+$('span.cid').text()+'/boards/'+$('span.bid').text()+'/folders/'+$('span.fid').text()+'/messages/'+mid+'/move/'+did+'?access_token='+$('span.token').text(),
			type: "POST",
			dataType: "json",
			data:"",
