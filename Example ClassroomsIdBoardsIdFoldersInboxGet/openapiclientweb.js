			...
			success: function(response) {
				...
				if (response) {
					...
					while (iter < response.boards.length){
						...
						row.cells[7].innerHTML = '<button onclick="getFolder('+(iter+1)+')">getFoldersfromBoard'+iter+'</button>';
						iter++;
					}
				}
			}
...
function getFolder(index){
	...
	$.ajax({
			...
			url: apiHost + '/classrooms/'+$('span.cid').text()+'/boards/'+$('span.bid').text()+'/folders/inbox?access_token='+$('span.token').text(),
			type: "GET",
			dataType: "json",
			success: function(response) {
