...
function getMessageL(index){
	...
	$.ajax({
			url: apiHost + '/subjects/'+$('span.cid').text()+'/boards/'+$('span.bid').text()+'/messages?access_token='+$('span.token').text(),
			type: "GET",
			dataType: "json",
			success: function(response) {
				...
				if (response) {
					...
					while (iter < response.messages.length){
						...
						row.cells[10].innerHTML = '<button onclick="getFolders('+(iter+1)+')">getDestinationofMessage'+iter+'</button>';
						iter = iter + 1;
					}
				}
				$('div.getBoardL').hide();
			...
			success: function(response) {
				...
				if (response) {
					...
					while (iter < response.folders.length){
						...
						row.cells[0].innerHTML = '<button onclick="moveMessage('+indexm+','+(iter+1)+')">moveMessage '+iter+'</button>';
						iter = iter + 1;
					}
				}
...
function moveMessage(indexm, indexf){
	var mid = document.getElementById("taulaMessageL").rows[indexm].cells[0].innerHTML;
	var fid = document.getElementById("taulaDesti").rows[indexf].cells[1].innerHTML;
    /*STARTUOCAPIEXAMPLE*/
    var missatge = '{}'
	$.ajax({
			/* UOCAPICALL /api/v1/subjects/{domain_id}/boards/{board_id}/messages/{id}/move/{folder_id} POST*/
			url: apiHost + '/subjects/'+$('span.cid').text()+'/boards/'+$('span.bid').text()+'/messages/'+mid+'/move/'+fid+'?access_token='+$('span.token').text(),
			type: "POST",
			dataType: "json",
			data:"",
