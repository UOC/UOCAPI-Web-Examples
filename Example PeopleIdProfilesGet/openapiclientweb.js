			...
			success: function(response) {
				...
				if (response) {
					...
					while (iter < response.people.length){
						...
						row.cells[17].innerHTML = '<button onclick="getProfiles('+(iter+1)+')"> Profiles '+iter+'</button>';						
						iter = iter + 1;
					}
				}
			}
...
function getProfiles(pid){
	...
	$.ajax({
			...
			url: apiHost + '/people/'+id+'/profiles?access_token='+$('span.token').text(),
			type: "GET",
			dataType: "json",
			success: function(response) {
				var table = document.getElementById("getpeopleidprofiles");
				...
				if (response) {
					...
					firstrow.cells[7].innerHTML = "<b>language</b>";
 					...
 					while (iter < n) {
						...
						//console.log(response.profiles[iter]);
						i=0;
